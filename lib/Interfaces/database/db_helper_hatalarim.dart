...
//DATABASE BAŞLATMA
  Future<Database> baslatDb() async {
    String dbPath = join(await getDatabasesPath(), "hatalarim.db");
    var listelerimDb =
...
//DATABASE OLUŞTURMA
  void olusturDb(Database db, int version) async {
    await db.execute("Create Table hatalarim(bolumadi string,bolum integer,sira integer,Primary key(bolumadi,bolum,sira));");
  }

// VERİ OKUMA
  Future<List<Kelime>> getVeri() async {
    Database? db = await this.db;
    var query = await db!.query("hatalarim");
...
// Veri EKLEME
  Future<int?> ekleVeri(Kelime kelime) async {
    Database? db = await this.db;
    List<Map?>? map=await db?.query("hatalarim",columns: ['bolum'],where: '"bolumadi"=? and "bolum"=? and "sira"=?',whereArgs: [kelime.bolumadi,kelime.bolum,kelime.sira]);
    //eğer verinin var olup olmadığını sorgulayorum
    if (map!.isEmpty) {
      var query = await db!.insert("hatalarim", kelime.toMap());
    return query;
...
// Veri Silme
  Future<int> silVeri(String bolumadi,int bolum, int sira) async {
    ...
    var query = await db!.rawDelete(
        'delete from hatalarim where "bolumadi"=? and "bolum"=? and "sira"=?',[bolumadi,bolum,sira]);
    return query;
  }
}
